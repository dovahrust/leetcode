...
struct ThroneInheritance {
    data: Vec<(Rc<str>, bool, Vec<usize>)>,
    hashmap: HashMap<Rc<str>, usize>,
}
...
impl ThroneInheritance {
    ...
    fn new(king_name: String) -> Self {
        let name: Rc<str> = Rc::from(king_name);
        let data = vec![(name.clone(), true, Vec::new())];
        let mut hashmap: HashMap<Rc<str>, usize> = HashMap::new();
        hashmap.insert(name, 0);
        Self {
            data: data,
            hashmap: hashmap,
        }
    ...
    fn birth(&mut self, parent_name: String, child_name: String) {
        let name: Rc<str> = Rc::from(child_name);
        let idx: usize = self.hashmap[&*parent_name];
        let child_id = self.data.len();
        self.data.push((name.clone(), true, Vec::new()));
        self.hashmap.insert(name, child_id);
    ...
    fn dfs(&self, curr: usize, res: &mut Vec<String>) {
        if self.data[curr].1 {
            res.push(self.data[curr].0.to_string());
        }
